from typing import Optional, MutableMapping, Any, Mapping, Sequence,Literal,List,Tuple
import argparse
...
def parse_command_line_arguments():
    ...
    # 添加 mode1 的子解析器
    mode1_parser = subparsers.add_parser("encode", help="Encode the input FASTA file into a numerical matrix using K-mer frequencies.",
                                         formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    mode1_parser.add_argument(
        "-q","--query", type=str, required=True,
        help="Path to the query (gzip) FASTA file."
        ) ##支持gzip/非gzip fasta文件
    mode1_parser.add_argument("-t","--target", type=str,  required=True, help="Path to the target (gzip) FASTA file.")
    mode1_parser.add_argument(
    ...
    mode1_parser.add_argument(
        "-p","--preprocess", type=str, required=False, default="IDF", 
        help="Preprocess method you want to implement to matrix.(TF/IDF/TF-IDF/None)"
        )
    mode1_parser.add_argument(
        "-k","--kmer-size", type=int, required=False, default=16, 
        help="K-mer size."
        )
    mode1_parser.add_argument(
        "--kmer-sample-fraction", type=float, required=False, default=0.005, 
    ...
    mode2_parser.add_argument(
        "-n","--n-dimensions", type=int,  required=False, 
        default=500,
        help="Dimension of feature vector after dimensionality reduction",
        )
    mode2_parser.add_argument(
        "--dm-config", type=str,  required=False, 
        default=None,
    ...
    mode2_parser.add_argument(
        ...
        default=20,
        help="Nearest neighbor number.",
        )
    mode2_parser.add_argument(
        "--knn-config", type=str,  required=False, 
        default=None,
...
def get_kw(
        args,
        kw_type:Literal['dimension_reduction','knn']
        )->Mapping[str,str] :
    """
...
def tfidf_qvt(
        tar_feature_matrix: np.ndarray,
        que_feature_matrix: np.ndarray,
        preprocess_type :str
    ...
    if preprocess_type == 'IDF':
        ...
        tar_train = transformer.fit_transform(tar_feature_matrix)
        que_fit =  transformer.transform(que_feature_matrix)
...
def tfidf_ava(feature_matrix: np.ndarray,
    ...
    if preprocess_type == 'IDF':
        ...
        pre_feature_matrix = TfidfTransformer(use_idf=True, smooth_idf=True).fit_transform(feature_matrix)
    elif preprocess_type == 'TF-IDF':
        pre_feature_matrix = TfidfTransformer(use_idf=True, smooth_idf=True).fit_transform(feature_matrix)
    elif preprocess_type == 'None':
        feature_matrix[feature_matrix > 0] = 1
...
def dimension_reduction_qvt(args) ->Tuple[np.ndarray,np.ndarray]:
    ...
    if dim == 'None':
        ...
        if dim in ['GaussianRP','SparseRP']:
            query_fm_dim = dimension_reduction_method().transform(query_fm,  args.n_dimensions, **dimension_reduction_kw)
            taget_fm_dim = dimension_reduction_method().transform(taget_fm,  args.n_dimensions, **dimension_reduction_kw)
        else:
            data =  sp.sparse.vstack([taget_fm,query_fm])
            data_dim = dimension_reduction_method().transform(data,  args.n_dimensions, **dimension_reduction_kw)
...
def dimension_reduction_ava(args) ->np.ndarray:
    dim = args.dimension_reduction_method
    target_fm_file = args.encode_dir + 'feature_matrix.npz'
    taget_fm = sp.sparse.load_npz(target_fm_file)
...
def get_neighbor(
        taget_fm_dim: np.ndarray,
        query_fm_dim: np.ndarray,
        args
        ) -> Tuple[np.ndarray,np.ndarray]:
    knn_method = args.knn_method
    if knn_method not in knn_str2func.keys():
        raise KeyError(f"Invalid k Neareast Neighbor method: {knn_method}. Expected one of {list(knn_str2func.keys())}.")
...
def main():
    ...
    if args.mode == "encode":
        ...
        if args.target == args.query:
            ...
            qread_names,fread_names,tar_feature_matrix,que_feature_matrix = encode_qvt(args)
            tar_train,que_fit = tfidf_qvt(tar_feature_matrix,que_feature_matrix,args.preprocess)
            logger.info("Saving output to files.")
        ...
        if args.dimension_reduction_method == 'None' and args.knn_method in ['HNSW','PQ','NNDescent']:
            logger.warning(f"Using {args.knn_method} in high-dimensional sparse matrix may consume significant time and memory.")
        if args.dimension_reduction_method != 'None' and args.knn_method in ['SimHash','MinHash']:
            logger.warning(f"{args.knn_method} can achieve higher accuracy without dimensonal reductionl.")

        ava_files_exist, ava_missing_files = check_files_exist([fm_file, name_file])
        qvt_files_exist, qvt_missing_files = check_files_exist([target_fm_file, query_fm_file, qname_file, fname_file])

        if ava_files_exist:
            logger.info("All required file exist, starting excuting dimensional reduction and knn neighbor finding.")
            ...
            df = format_output(neighbor_indices,distance,target_read_name,query_read_name)
        else:
